...
init_particles = function(time,sx,sy,vx,vy,vt){
  particles = data.frame(time, c(seq(1,n_particles,by=1)), sx, sy,vx,vy,vt)
  names(particles) <- c('time', 'particle','Sx', 'Sy','Vx','Vy','vTotal')
  return(particles)
}
...
box_n_n = function(dt, time_max, particles){
  time = 0
    while(time <= time_max){
      ...
      particles <- rbind(particles,init_particles(rep(time, n_particles), sx, sy,vx,vy,sum(sqrt((vx^2)+(vy^2)))))
    }
  
  #Se ordena la partícula en base a un identificador
  #particles = particles[order(particles$particle), ]
  lastvel<- sqrt((vx^2)+(vy^2))
  return (particles)
} 
...
simu_one_box <- function(){
  particles = init_particles(rep(0, n_particles), sx,sy,vx,vy,sum(sqrt((vx^2)+(vy^2))))
